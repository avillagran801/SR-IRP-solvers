#include "BeamSearchPrevProfit.hpp"
      ...
      daily_solver::SolverAlgorithm daily_algorithm)
  {
    ...
    for (int i = 0; i < instance.T; i++)
    {
      ...
      for (int j = 0; j < num_runs; j++)
      {
        ...
        // In this version we also pass the previous node accumulated profit
        int prev_profit;
      ...
      // Sort descending by its total accumulated profit
      std::sort(current_day_pool.begin(), current_day_pool.end(),
                [](const DayNodeSolution &a, const DayNodeSolution &b)
                {
                  return a.decoded_solution.total_accumulated_profit > b.decoded_solution.total_accumulated_profit;
                });
