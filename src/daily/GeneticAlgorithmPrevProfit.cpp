#include "GeneticAlgorithmPrevProfit.hpp"
...
namespace daily_solver
{
  ...
  namespace
  {
    class DailyReplenishmentFitnessPrevProfit : public gapp::FitnessFunctionBase<gapp::PermutationGene>
    {
    private:
    ...
    public:
      DailyReplenishmentFitnessPrevProfit(const types::DayContext &context, const routing::DecoderInterface &dec)
      ...
      gapp::FitnessVector invoke(const gapp::Candidate<gapp::PermutationGene> &solution) const override
      {
        ...
        // In this version the fitness value is the total accumulated profit
        double fitness_value = static_cast<double>(result.total_accumulated_profit);
    ...
    void configureGA(gapp::GA<gapp::PermutationGene> &ga, const GAConfigPrevProfit &config)
    {
      ga.max_gen(10000);
  ...
  GeneticAlgorithmPrevProfit::GeneticAlgorithmPrevProfit(GAConfigPrevProfit config, const routing::DecoderInterface &decoder)
      : config(config), decoder(decoder) {}

  std::vector<DailyResult> GeneticAlgorithmPrevProfit::solve(const types::DayContext &ctx, double time_limit_seconds) const
  {
    ...
    DailyReplenishmentFitnessPrevProfit problem(ctx, decoder);
