...
namespace daily_solver
{
  ...
  ACO::ACO(ACOConfig config, const routing::DecoderInterface &decoder)
      : config(config), decoder(decoder) {}

  std::vector<DailyResult> ACO::solve(const types::DayContext &ctx, double time_limit_seconds) const
  {
    ...
    while (true)
    {
      ...
      for (int i = 0; i < config.num_ants; i++)
      {
        ...
        {
          // CHANGE LATER
          worst_current_solution_value = decoded_solutions[i].total_day_utility;
          best_current_solution_value = decoded_solutions[i].total_day_utility;
        }
        else
        {
          if (decoded_solutions[i].total_day_utility < worst_current_solution_value)
          {
            worst_current_solution_value = decoded_solutions[i].total_day_utility;
          }
          if (decoded_solutions[i].total_day_utility > best_current_solution_value)
          {
            best_current_solution_value = decoded_solutions[i].total_day_utility;
          }
        }
        ...
        if (!is_duplicate)
        {
          ...
          if (elite_sequences.size() < elite_sequences_size)
          {
            ...
            std::sort(elite_sequences.begin(), elite_sequences.end(), [](const DailyResult &a, const DailyResult &b)
                      { return a.solution.total_day_utility > b.solution.total_day_utility; });
          }
          else
          {
            if (current_result.solution.total_day_utility > elite_sequences.back().solution.total_day_utility)
            {
      ...
      {
        double current_best_utility = elite_sequences.front().solution.total_day_utility;
      ...
      for (int i = 0; i < config.num_ants; i++)
      {
        ...
        double deposit = config.q * ((decoded_solutions[i].total_day_utility - worst_current_solution_value + 0.001) /
