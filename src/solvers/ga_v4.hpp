...
{
  /**
   * @brief Executes the Version 4 Genetic Algorithm (GA) solver.
   *
   ...
   * @param beam_width Number of candidate states to preserve per day in the Beam Search (default: 1).
   * @param future_effect_weight Multiplier determining how heavily the potential future missing
   *                             inventory penalty affects the day's sorting score (default 0.78).
   ...
   * @param crossover Identifier for the crossover operator to use (0: Order1, 1: Order2, 2: Position, 3: Edge, 4: Edge, 5: Partially Mapped) (default 0).
   * @param mutation Identifier for the mutation operator to use (0: Inversion, 1: Swap2, 2: Swap3, 3: Shuffle, 4: Shift) (default 1).
   * @param crossover_rate Probability (0.0 to 1.0) of applying crossover to selected parents (default 0.90).
   ...
   */
  common::SolverResult RunGAV4(
      std::string instance_name,
      int max_time_seconds = 90,
      int beam_width = 1,
      float future_effect_weight = 0.78,
