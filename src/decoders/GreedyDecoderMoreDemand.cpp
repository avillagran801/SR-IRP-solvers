#include "decoders/GreedyDecoderMoreDemand.hpp"
#include <algorithm>
#include <numeric>

namespace routing
{
  types::DaySolution GreedyDecoderMoreDemand::decode(
      ...
      const types::DayContext &ctx) const
  {
    ...
    for (std::size_t i = 0; i < seq_size; i++)
    {
      ...
      // Sort the indices based on the product demand in descending order
      std::stable_sort(ordered_product_index.begin(), ordered_product_index.end(), [&client_product_demands](size_t left, size_t right)
                       { return client_product_demands[left] > client_product_demands[right]; });
