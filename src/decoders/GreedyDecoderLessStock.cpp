...
namespace routing
{
  types::DaySolution GreedyDecoderLessStock::decode(
      ...
      const types::DayContext &ctx) const
  {
    ...
    for (std::size_t i = 0; i < seq_size; i++)
    {
      ...
      std::vector<int> client_product_start_inventory = ctx.start_inventory[client_id];
      std::vector<int> client_product_demands = ctx.demand[client_id];

      // Initialize product index vector
      std::vector<std::size_t> ordered_product_index(client_product_demands.size());
      std::iota(ordered_product_index.begin(), ordered_product_index.end(), 0);

      // Sort the indices based on what product will have less stock at the end of the day
      std::stable_sort(ordered_product_index.begin(), ordered_product_index.end(), [&client_product_start_inventory, &client_product_demands](size_t left, size_t right)
                       { return (client_product_start_inventory[left] - client_product_demands[left]) < (client_product_start_inventory[right] - client_product_demands[right]); });

      // For each product based on its demand
      for (int j = 0; j < ordered_product_index.size(); j++)
      {
        int product_id = ordered_product_index[j];

        int amount_to_restock = ctx.demand[client_id][product_id];
        ...
        float service_time = amount_to_restock * ctx.restock_time[product_id];
        ...
        while (active_worker < ctx.num_workers)
        {
          ...
          if (required_time_with_return > ctx.L)
          {
            ...
            int possible_restock_by_time = available_time / ctx.restock_time[product_id];

            if (possible_restock_by_time > 0)
            {
              amount_to_restock = possible_restock_by_time;

              service_ending_time = service_starting_time + amount_to_restock * ctx.restock_time[product_id];
          ...
          if (service_ending_time > ctx.finishing_hour[client_id])
          {
          ...
          {
            result.routes[active_worker].back().restocked_product[product_id] = amount_to_restock;
          ...
          else
          {
            ...
            new_visit.restocked_product[product_id] = amount_to_restock;
    ...
    for (int i = 0; i < ctx.num_workers; i++)
    {
      ...
      for (int j = 0; j < result.routes[i].size(); j++)
      {
        ...
        for (int k = 0; k < ctx.num_products; k++)
        {
          ...
          int possible_aditional_restock_by_capacity = ctx.max_inventory[client_id][k] - current_inventory_with_restock;

          int possible_aditional_restock_by_time = available_time / ctx.restock_time[k];

          int aditional_restock = std::min(possible_aditional_restock_by_capacity, possible_aditional_restock_by_time);

          if (aditional_restock > 0)
          {
            result.routes[i][j].restocked_product[k] += aditional_restock;

            float aditional_time_spent = aditional_restock * ctx.restock_time[k];

            result.workers_total_time[i] += aditional_time_spent;
            result.routes[i][j].visit_ending_time += aditional_time_spent;
            available_time -= aditional_time_spent;

            if (result.routes[i][j].visit_ending_time > ctx.finishing_hour[client_id])
            {
              result.missed_schedule[client_id] = true;
            }
          }
    ...
    // 3. UPDATE THE INVENTORY AND UTILITIES
    for (int i = 0; i < ctx.num_workers; i++)
    {
      for (int j = 0; j < result.routes[i].size(); j++)
      {
    ...
    result.total_accumulated_profit = ctx.prev_utilities + result.total_day_utility;
